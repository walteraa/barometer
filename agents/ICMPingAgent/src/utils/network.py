...
def send_ping(aggregate, target, callback):
    ...
    ping_response = 4 if ping_response is None else ping_response

    callback(aggregate, ping_response, target, {
             'sensor': 'PING', 'metric': 'ping-time'})
...
def http_callback(aggregate, monitoring_data, target, agent_type):
    ...
    print("sending HTTP to: %s:%d with data: %s" %
          (aggregate['host'], aggregate['port'], json.dumps(json_map)))
