...
def create_job(conf):
    def calc_average():
        ...
        pipeline = [
            ...
            {'$group': {'_id': {'cluster_id': '$cluster_id',  'metric_name': '$metric_name',
                                'value_type': '$value_type'}, 'count': {'$sum': 1}, 'value': {'$avg': '$metric_value'}}}
        ]
        ...
        if len(data) > 0:
            data = data[0]
            http_callback(conf.get_aggregate()[
                          'cluster_id'], conf.get_barograph(), data)
